...
function wordsList(str, subStr) {
    str = str.replace(/[.,!?;:"]/g, '');
    let arr = str.toLowerCase().split(' ').filter((itemArr) => itemArr.indexOf(subStr) > -1);
    let newSet = new Set();
    arr.forEach(itemArr => {
        newSet.add(itemArr);
    });
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
    ...
    if (!isISO) restore = isSeconds ? date.toLocalString() : date.toLocalString().replace(regexp, '');
    else {
        year = year;
        ...
        restore = isSeconds ? year + "-" + month + '-' + 'day' + ', ' + minutes + ':' + seconds : year + "-" + month + '-' + 'day' + ', ' + minutes;
    }
    return restore;
}
