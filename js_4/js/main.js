...
function wordsList(str, subStr) {
  ...
  let arr = str
    .toLowerCase()
    .split(" ")
    .filter((itemArr) => itemArr.indexOf(subStr) > -1);
  let newSet = new Set();
  arr.forEach((itemArr) => {
    newSet.add(itemArr);
  });
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
  const regexp = new RegExp(":\\d{2}$", "gui");
  let restore;
...
console.log(getLocalDay("2022-01-24"));
console.log(razdel);
/*
 * #5
 *
...
let getDateAgo = (date, days) => {
  ...
  dateN.setDate(dateN.getDate() - days);
  return dateN.toLocaleString().replace(/(\d.*),\s+(\d.*)/gu, "$1");
};
